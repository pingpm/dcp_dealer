  ...
  if (interceptor && Object.keys(interceptor).length) {
    if (isArray(interceptor.invoke)) {
      ...
      return res.then((options2) => {
        return api2(wrapperOptions(getApiInterceptorHooks(method), options2), ...params);
      });
    } else {
...
function promisify(name, api2) {
  ...
  return function promiseApi(options = {}, ...rest) {
    if (isFunction(options.success) || isFunction(options.fail) || isFunction(options.complete)) {
      return wrapperReturnValue(name, invokeApi(name, api2, extend({}, options), rest));
    }
    return wrapperReturnValue(name, handlePromise(new Promise((resolve2, reject) => {
      invokeApi(name, api2, extend({}, options, {
        success: resolve2,
        fail: reject
      }), rest);
    })));
...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__C656B3F",
    appName: "dealer-uniapp",
    appVersion: "1.0.0",
    appVersionCode: "100",
...
const createSSRApp = createApp$2;
...
if (!Math) {
  "../pages/home/index.js";
  "../pages/auth/login.js";
  "../pages/agreement/detail.js";
  "../pages/verification/status.js";
  "../pages/verification/form.js";
...
const MINIAPP_API_BASE_URL = "http://127.0.0.1:3001/api";
let apiBaseUrl = MINIAPP_API_BASE_URL;
const API_BASE_URL = apiBaseUrl;
const TOKEN_KEY = "dealer_token";
const PROFILE_KEY = "dealer_profile";
function getToken() {
  return index.getStorageSync(TOKEN_KEY) || "";
}
function setSession(data) {
  index.setStorageSync(TOKEN_KEY, data.token);
  index.setStorageSync(PROFILE_KEY, data);
}
function clearSession() {
  index.removeStorageSync(TOKEN_KEY);
  index.removeStorageSync(PROFILE_KEY);
}
function getSession() {
  return index.getStorageSync(PROFILE_KEY) || null;
}
function requireLogin() {
  if (!getToken()) {
    index.reLaunch({ url: "/pages/auth/login" });
    return false;
  }
  return true;
}
  ...
  return new Promise((resolve2, reject) => {
    index.request({
      ...
      success(res) {
        ...
        index.showToast({ title: message, icon: "none" });
        reject({ ...body, statusCode: res.statusCode, message });
      },
      fail(error) {
        index.showToast({ title: "网络连接失败", icon: "none" });
        reject(error);
      }
    });
  });
...
function uploadFile(filePath, fileType, usageScene = "") {
  ...
  return new Promise((resolve2, reject) => {
    index.uploadFile({
      ...
      fail(error) {
        index.showToast({ title: "上传失败", icon: "none" });
        reject(error);
      }
    });
  });
...
async function ensureDealerReady() {
  ...
  try {
    ...
    if (status.reviewStatus !== "APPROVED") {
      index.redirectTo({ url: "/pages/verification/status" });
      return false;
    }
    return true;
  } catch (error) {
...
function goAfterLogin(loginData) {
  if (loginData == null ? void 0 : loginData.needVerificationPrompt) {
    index.setStorageSync("dealer_need_verification_prompt", "1");
  }
  index.reLaunch({ url: "/pages/home/index" });
}
